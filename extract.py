...
class Extractor:
    ...
    def _collect_reachable_nodes(
        ...
        for name in output_names:
            self._dfs_search_reachable_nodes(
                name, input_names, reachable_nodes)
    ...
    def _collect_referred_local_functions(
        ...
        while new_nodes:
            new_nodes = find_referred_funcs(
                new_nodes, referred_local_functions)
    ...
    def _collect_reachable_tensors(
        ...
        initializer = [self.wmap[t]
                       for t in self.wmap if t in all_tensors_names]
        value_info = [self.vimap[t]
                      for t in self.vimap if t in all_tensors_names]
